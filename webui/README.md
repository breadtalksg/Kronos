...
## BTCUSDT Forecast Demo

This research-only demo reads public BTCUSDT K-line data from Binance and forecasts the next 24 periods with a lightweight Kronos model. It does not connect to a broker or execute orders.
...
```shell
python3 scripts/forecast_btcusdt.py --symbol BTCUSDT --interval 1h --lookback 500 --pred-steps 24 --model mini --device auto
```

The generated files are:

```text
outputs/btcusdt_forecast.csv
outputs/btcusdt_forecast.png
```

Kronos forecast is for research and learning only. It is not financial advice and should not be used as an automatic trading signal.

## BTCUSDT Forecast Web UI
...
```shell
streamlit run webui/btc_forecast_app.py
```

Open the browser page and click **Run Forecast**. The dashboard displays the current parameters, forecast metrics, a Chinese summary, the prediction table, and the forecast chart.

All page timestamps use Singapore Time / UTC+8. Model output is for research only and does not constitute investment advice.
